...
class SettingsPage extends StatelessWidget {
  ...
  Future<void> change(BuildContext context, String key, bool value) async {
    try {
      if (key == 'reminders' && value && !await reminders.requestPermission())
        throw StateError(
          'Notification or exact-alarm permission was not granted. Enable it in device settings and retry.',
        );
      if (key == 'appLock' && !await authenticate())
        throw StateError(
          'Authentication was not completed. Check your device passcode or biometric settings and retry.',
        );
      await store.update((r) => r.settings[key] = value);
      if (key != 'appLock') await reminders.sync(store.records);
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    final settings = store.records.settings;
    return ListView(
      padding: const EdgeInsets.all(20),
      children: [
        const Section(
            ...
            children: [
              SwitchListTile(
                ...
                onChanged: store.busy || reminders.syncing
                    ? null
                    : (v) => change(context, 'reminders', v),
              ),
              SwitchListTile(
                contentPadding: EdgeInsets.zero,
                title: const Text('Private notification previews'),
                subtitle: const Text(
                  'Keep medication names off reminder messages.',
                ),
                value: settings['privateNotifications'] != false,
                onChanged: store.busy || reminders.syncing
                    ? null
                    : (v) => change(context, 'privateNotifications', v),
              ),
              if (reminders.error != null) Notice(reminders.error!),
              if (settings['reminders'] == true)
                Text(
                  reminders.through == null
                      ? 'Scheduling has not been confirmed.'
                      : '${reminders.count} doses queued through ${dayKey(reminders.through!)} at ${clockLabel(reminders.through!.hour * 60 + reminders.through!.minute)}. Timezone: ${reminders.zone}.',
                ),
              const Notice(
                'Open DoseBuddy regularly to refresh reminders. Up to 60 doses are queued, at most 30 days ahead. Device permissions and settings can affect delivery.',
              ),
              Wrap(
                spacing: 12,
                runSpacing: 12,
                children: [
                  OutlinedButton(
                    onPressed: reminders.syncing
                        ? null
                        : () => reminders.sync(store.records),
                    child: Text(
                      reminders.syncing ? 'Refreshing…' : 'Refresh reminders',
                    ),
                  ),
                  OutlinedButton(
                    onPressed: settings['reminders'] != true
                        ? null
                        : () async {
                            try {
                    ...
                    child: const Text('Send one-time test'),
                  ),
                ],
              ),
            ],
        ...
        Section(
          ...
          Column(
            children: [
              SwitchListTile(
                contentPadding: EdgeInsets.zero,
                title: const Text('Lock with device authentication'),
                subtitle: const Text(
                  'Require Face ID, fingerprint or device passcode when returning to the app.',
                ),
                value: settings['appLock'] == true,
                onChanged: store.busy
                    ? null
                    : (v) => change(context, 'appLock', v),
              ),
              ListTile(
                contentPadding: EdgeInsets.zero,
                title: const Text('Privacy notice'),
                trailing: const Icon(Icons.chevron_right),
                onTap: () => textPage(context, 'Privacy notice', privacyText),
              ),
              ListTile(
                contentPadding: EdgeInsets.zero,
                title: const Text('Using DoseBuddy'),
                trailing: const Icon(Icons.chevron_right),
          ...
          Column(
            children: [
              ListTile(
                ...
                subtitle: const Text(
                  'Review or share a personal medication card.',
                ),
                trailing: const Icon(Icons.chevron_right),
                onTap: () => Navigator.push(
                  context,
                  MaterialPageRoute<void>(
                    builder: (_) => EmergencyPage(store: store),
                  ),
                ),
              ),
              ListTile(
                contentPadding: EdgeInsets.zero,
                title: const Text('Review and export records'),
                trailing: const Icon(Icons.ios_share),
                onTap: () => Navigator.push(
                  context,
                  MaterialPageRoute<void>(
                    builder: (_) => ExportPage(store: store),
                  ),
                ),
              ),
              ListTile(
                ...
                subtitle: const Text(
                  'Includes history, preferences and reminders.',
                ),
                onTap: store.busy
                    ? null
                    : () async {
                        final yes = await showDialog<bool>(
                          ...
                          builder: (ctx) => AlertDialog(
                            ...
                            content: const Text(
                              'This cannot be undone. All medication records, symptoms, emergency information and correction history will be removed. Previously exported copies remain outside the app.',
                            ),
                            actions: [
                              TextButton(
                        ...
                        try {
                          ...
                          reminders.pendingAction = null;
                          await store.erase();
                          if (context.mounted)
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text(
                                  'All local records and reminders removed.',
                                ),
                              ),
                            );
                        } catch (e) {
        ...
        OutlinedButton.icon(
          onPressed: () async {
            final ok = await launchUrl(
              Uri.parse('https://github.com/omuoguilim/DoseBuddy/issues'),
              mode: LaunchMode.externalApplication,
            );
            if (!ok && context.mounted)
              showError(
                context,
                'Could not open support. Visit github.com/omuoguilim/DoseBuddy/issues.',
              );
          },
          icon: const Icon(Icons.help_outline),
          label: const Text('Support and feedback'),
        ),
        const Padding(
          padding: EdgeInsets.only(top: 12),
          child: Text(
            'Do not include private health information in public feedback.',
...
String exportCsv(Records r) {
  final lines = <String>[
    [
      'record_type',
      'medication_or_symptom',
      'strength',
      'scheduled_time',
      'actual_time',
      'status',
  ...
  for (final value in r.outcomes.values) {
    final e = value as Json;
    final d = ScheduledDose.fromSnapshot(e['dose'] as Json);
    lines.add(
      [
        e['prn'] == true ? 'as_needed' : 'scheduled',
        d.details['name'],
        d.details['strength'],
  ...
  for (final value in (r.data['symptoms'] as Json).values) {
    final s = value as Json;
    lines.add(
      [
        'symptom',
        s['name'],
...
class ExportPage extends StatefulWidget {
  final AppStore store;
  const ExportPage({super.key, required this.store});
  @override
...
class _ExportPageState extends State<ExportPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final text = type == 'JSON'
        ? const JsonEncoder.withIndent('  ').convert(r.data)
        : type == 'CSV'
        ? exportCsv(r)
        : r.report(DateTime(now.year, now.month, now.day - 29), now);
    return Scaffold(
      appBar: AppBar(title: const Text('Review export')),
      body: ListView(
        padding: const EdgeInsets.all(20),
        children: [
          DropdownButtonFormField<String>(
            ...
            items: const [
              DropdownMenuItem(value: 'Summary', child: Text('30-day summary')),
              DropdownMenuItem(
                value: 'CSV',
                child: Text('Recorded doses and symptoms (CSV)'),
              ),
              DropdownMenuItem(
                value: 'JSON',
                child: Text('Complete records and corrections (JSON)'),
              ),
            ],
            onChanged: (s) => setState(() => type = s!),
          ),
          const Notice(
            'This file contains health information and is not encrypted. Review it before choosing a recipient. Shared copies cannot be revoked.',
          ),
          ...
          Builder(
            builder: (buttonContext) => FilledButton.icon(
              onPressed: busy
                  ...
                  : () async {
                      ...
                      try {
                        ...
                        final file = File(
                          '${dir.path}/dosebuddy-${dayKey(now)}.${type == 'JSON'
                              ? 'json'
                              : type == 'CSV'
                              ? 'csv'
                              : 'txt'}',
              ...
              label: const Text('Choose where to share'),
            ),
          ),
        ],
      ),
...
class EmergencyPage extends StatefulWidget {
  final AppStore store;
  const EmergencyPage({super.key, required this.store});
  @override
...
class _EmergencyPageState extends State<EmergencyPage> {
  ...
  void initState() {
    super.initState();
    final e = widget.store.data['emergency'] as Json;
    allergies.text = e['allergies'] as String;
    contact.text = e['contact'] as String;
    notes.text = e['notes'] as String;
  }
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: ListView(
      ...
      children: [
        const Notice(
          'This is a personal information card inside DoseBuddy. It does not appear on your phone’s emergency lock screen and does not contact emergency services.',
        ),
        TextField(
          controller: allergies,
          maxLines: 2,
          decoration: const InputDecoration(
            labelText: 'Known allergies or explicit “none known”',
          ),
        ),
        ...
        TextField(
          controller: contact,
          decoration: const InputDecoration(
            labelText: 'Emergency contact and phone',
          ),
        ),
        const SizedBox(height: 16),
        TextField(
          controller: notes,
          maxLines: 3,
          decoration: const InputDecoration(labelText: 'Other information'),
        ),
              ...
              : () async {
                  ...
                  try {
                    ...
                    if (context.mounted)
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Emergency information saved.'),
                        ),
                      );
                  } catch (e) {
                    if (context.mounted) showError(context, e);
        ...
        OutlinedButton(
          onPressed: () {
            final r = widget.store.records;
            final medications = r.medications.values
                .cast<Json>()
                .map(r.current)
                .where(
                  (f) =>
                      f['active'] == true &&
                      (f['start'] as String).compareTo(dayKey(DateTime.now())) <= 0 &&
                      (f['end'] == null ||
                          (f['end'] as String).compareTo(
                                dayKey(DateTime.now()),
                              ) >=
                              0),
                )
                ...
                builder: (_) => Scaffold(
                  ...
                  body: ListView(
                    ...
                    children: [
                      SelectableText(text),
                      const Notice(
                        'Sharing sends a copy of all information shown above.',
                      ),
                      Builder(
          ...
          child: const Text('Review card before sharing'),
        ),
      ],
    ),
  );
