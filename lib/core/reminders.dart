...
class Reminders extends ChangeNotifier {
  ...
  Future<void> initialize() async {
    ...
    if (launch?.didNotificationLaunchApp ?? false)
      pendingAction = launch?.notificationResponse;
    initialized = true;
  }
  ...
  NotificationDetails details({bool actions = true}) => NotificationDetails(
    android: AndroidNotificationDetails(
      'dosebuddy_private_v2',
      'Medication reminders',
      channelDescription: 'Reminders you scheduled in DoseBuddy',
  ...
  Future<void> _sync(Records r) async {
    ...
    try {
      ...
      zone = await FlutterTimezone.getLocalTimezone();
      tz.setLocalLocation(tz.getLocation(zone!));
      final now = DateTime.now();
      final plan = ReminderPlan.create(r, now);
      var id = 1000;
      for (final dose in plan.doses) {
        ...
        if (local.hour != minute ~/ 60 || local.minute != minute % 60)
          throw StateError(
            'A time does not exist after a daylight-saving change. Review your schedule.',
          );
        final private = r.settings['privateNotifications'] != false;
        await plugin.zonedSchedule(
          id++,
          private ? 'DoseBuddy reminder' : dose.details['name'] as String,
          'Open DoseBuddy to review your scheduled record.',
          local,
          details(),
      ...
      if (renew.isAfter(now))
        await plugin.zonedSchedule(
          900001,
          'Refresh DoseBuddy reminders',
          'Open the app to schedule the next set of reminders.',
          tz.TZDateTime.from(renew, tz.local),
          details(actions: false),
          androidScheduleMode: AndroidScheduleMode.exactAllowWhileIdle,
          uiLocalNotificationDateInterpretation:
              UILocalNotificationDateInterpretation.absoluteTime,
        );
      ...
      error =
          'Records are saved, but reminders could not be fully scheduled. Check notification and exact-alarm permissions, then retry.';
      through = null;
    } finally {
      syncing = false;
      notifyListeners();
  ...
  Future<void> snooze(String key) async {
    ...
    await plugin.zonedSchedule(
      900002,
      'DoseBuddy reminder',
      'Open DoseBuddy to review your dose record.',
  ...
  Future<void> test() async {
    ...
    await plugin.show(
      900003,
      'DoseBuddy test',
      'This is a one-time test notification.',
      details(actions: false),
    );
  }
}
