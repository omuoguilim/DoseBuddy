...
class AppStore extends ChangeNotifier {
  ...
  Future<void> erase() => update((r) {
    r.data.clear();
    r.data.addAll(Records.empty());
  });
  ...
  static Future<AppStore> open() async {
    ...
    if (key == null) {
      if (await Hive.boxExists('dosebuddy_v2'))
        throw StateError(
          'The local encryption key is missing. Existing data was not overwritten.',
        );
      key = base64Encode(Hive.generateSecureKey());
      await secure.write(key: 'dosebuddy.v2.key', value: key);
    }
    ...
    if (!box.containsKey('state')) {
      ...
      await box.put('state', jsonEncode(initial));
      await box.flush();
      if (jsonEncode(jsonDecode(box.get('state')!)) != jsonEncode(initial))
        throw StateError(
          'Import verification failed. Original data was retained.',
        );
    }
    ...
    if (oldImage != null) {
      ...
      if (roots.any((d) => oldImage.startsWith('${d.path}/'))) {
    ...
    return AppStore(decoded, (next) async {
      await box.put('state', jsonEncode(next));
      await box.flush();
      await box.compact();
    });
  }
  ...
  static Future<Json> importLegacy() async {
    ...
    if (await Hive.boxExists('medications')) {
      ...
      for (final m in box.values) {
        final fields = <String, dynamic>{
          'name': m.name,
          'strength': m.dosage,
          'amount': m.pillsPerDose,
          'unit': m.dosageForm.toLowerCase() == 'tablet'
              ? 'tablet'
              : m.dosageForm.toLowerCase() == 'capsule'
              ? 'capsule'
              : 'unit (verify)',
          'form': m.dosageForm,
          'type': m.medicationType,
          'times': m.times.map(parseClock).toList(),
          'start': dayKey(m.startDate ?? m.createdAt),
          'end': m.endDate == null ? null : dayKey(m.endDate!),
          'active': m.lifecycleStatus == 'active',
        ...
        r.medications[m.id] = {
          'id': m.id,
          'versions': [fields],
          'supply': m.pillsRemaining,
          'threshold': m.refillThreshold,
        };
        for (final log in m.takenLog ?? <Map<String, dynamic>>[]) {
          final date = DateTime.parse(log['date'] as String);
          final minute = parseClock(log['scheduledTime'] as String);
          final d = ScheduledDose(
            doseKey(m.id, date, minute),
            m.id,
            DateTime(
              date.year,
              date.month,
          ...
          r.outcomes[d.key] = {
            ...
            'status': 'Taken',
            'takenAt': log['takenAt'],
            'recordedAt': log['takenAt'] ?? date.toIso8601String(),
            'amount': m.pillsPerDose,
            'reason': 'Imported record; original schedule version unavailable.',
            'prn': false,
            'inventoryDeduction': 0,
            'inventoryReconciled': true,
          };
        }
        if (m.sideEffects?.isNotEmpty ?? false)
          (data['audit'] as List).add({
            'action': 'legacy side effects imported',
            'medicationId': m.id,
            'records': m.sideEffects,
          });
      }
      await box.close();
      (data['migrationNotes'] as List).add(
        'Old schedule versions were not stored. Recorded doses are preserved, but unrecorded scheduled totals before ${dayKey(now)} cannot be reconstructed reliably. Imported supply counts were preserved.',
      );
    }
    if (await Hive.boxExists('dose_events')) {
      ...
      for (final e in box.values) {
        ...
        final f = med == null
            ? <String, dynamic>{
                'name': 'Medication no longer listed',
                'strength': 'Unknown',
                'amount': e.amount,
                'unit': 'unit (verify)',
              }
            : r.current(med);
        final prn = e.id.startsWith('prn_');
        final key = prn
            ? e.id
            : doseKey(
                e.medicationId,
                e.scheduledAt,
                e.scheduledAt.hour * 60 + e.scheduledAt.minute,
              );
        final d = ScheduledDose(key, e.medicationId, e.scheduledAt, f);
        (data['audit'] as List).add({
          'action': 'legacy dose event imported',
        ...
        if (['taken', 'late', 'skipped'].contains(e.status))
          r.outcomes[key] = {
            'dose': d.snapshot(),
            'status': e.status == 'skipped' ? 'Skipped' : 'Taken',
            'takenAt': e.takenAt?.toIso8601String(),
            'recordedAt': (e.takenAt ?? e.scheduledAt).toIso8601String(),
            'amount': e.amount,
            'reason': e.reason ?? e.notes ?? 'Imported record',
            'prn': prn,
            'inventoryDeduction': 0,
            'inventoryReconciled': true,
          };
    ...
    if (await Hive.boxExists('symptom_events')) {
      ...
      for (final s in box.values) {
        (data['symptoms'] as Json)[s.id] = {
          'id': s.id,
          'name': s.name,
          'severity': s.severity,
          'at': s.occurredAt.toIso8601String(),
    ...
    return jsonDecode(
          jsonEncode(
            ...
            toEncodable: (value) {
              ...
              throw UnsupportedError('Unsupported legacy record value');
            },
          ),
        )
        as Json;
