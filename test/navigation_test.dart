...
void main() {
  test(
    'CSV quoting preserves commas and quotes and escapes formula prefixes',
    () {
      expect(csvCell('a,"b"'), '"a,""b"""');
      expect(csvCell('=1+1'), '"\'=1+1"');
    },
  );
  testWidgets('PRN-only routine is visible and opens a real record form', (
    tester,
  ) async {
    final data = Records.empty();
    Records(data).saveMedication(
      'prn',
      prescription(type: 'as_needed', times: []),
      DateTime.now(),
    );
    ...
    await tester.tap(find.text('Example medication · 10 mg'));
    await tester.pumpAndSettle();
    expect(find.text('Record dose'), findsOneWidget);
    expect(find.text('Save record'), findsOneWidget);
    expect(tester.takeException(), isNull);
  });
  testWidgets('history is reachable and shows calendar and export', (
    tester,
  ) async {
  ...
  testWidgets('empty Today remains usable with large text on a narrow screen', (
    ...
    expect(find.text('No scheduled doses today'), findsOneWidget);
    expect(tester.takeException(), isNull);
  });
}
